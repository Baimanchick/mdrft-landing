...
export function PartnershipModal({ onClose }: PartnershipModalProps) {
        ...
        {!submitted ? (
          <form onSubmit={handleSubmit} className="relative z-10 space-y-6 text-sm">
            ...
            <div className="space-y-3">
              <span
                id={`${titleId}-direction-label`}
                className="block text-xs uppercase tracking-wider text-[#8e8e93] font-semibold"
              >
                {isRu ? "Выберите направление сотрудничества *" : "Select Partnership Direction *"}
              </span>
              <div
                role="radiogroup"
                aria-labelledby={`${titleId}-direction-label`}
              ...
              >
                {PARTNERSHIP_OPTIONS.map((opt) => {
                  ...
                  return (
                    <label
                      key={opt.id}
                      className={`p-4 sm:p-5 rounded-xl border transition-all duration-200 cursor-pointer flex flex-col justify-between text-left relative overflow-hidden group has-[:focus-visible]:outline has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-offset-2 has-[:focus-visible]:outline-[#ea4043] ${
                        isSelected
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label htmlFor={`${titleId}-company`} className="block text-xs uppercase tracking-wider text-[#8e8e93] font-semibold mb-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label htmlFor={`${titleId}-contactMethod`} className="block text-xs uppercase tracking-wider text-[#8e8e93] font-semibold mb-2">
                  {isRu ? "Telegram или Email" : "Telegram or Email"}
                </label>
                <input
                  id={`${titleId}-contactMethod`}
                  name="contactMethod"
                  autoComplete="off"
                  type="text"
            ...
            {/* Comment / Details */}
            <div>
              <label htmlFor={`${titleId}-comment`} className="block text-xs uppercase tracking-wider text-[#8e8e93] font-semibold mb-2">
