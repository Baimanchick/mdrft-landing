...
export function CertificateModal({ onClose }: CertificateModalProps) {
  ...
  const DELIVERY_OPTIONS = [
    {
      id: "digital" as const,
      title: isRu ? "Электронный (PDF)" : "Digital PDF",
      hint: isRu ? "Мгновенно на Email" : "Instant email delivery",
    },
    {
      id: "box" as const,
      title: isRu ? "Подарочный бокс" : "Gift Box",
      hint: isRu ? "Металлическая карта в боксе" : "Metal card with delivery",
    },
  ];
  ...
  return (
    <div {...dialogProps} className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 md:p-8 overflow-y-auto no-scrollbar font-sans">
      {/* Deep Blur Backdrop */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
              ...
              <div className="lg:col-span-5 space-y-6">
                ...
                <div className="space-y-2 pr-6">
                  ...
                  <h3 id={titleId} className="text-2xl sm:text-3xl lg:text-4xl font-bold uppercase tracking-tight text-white leading-tight">
              ...
              <div className="lg:col-span-7 space-y-5">
                ...
                <div>
                  ...
                  <div
                    role="radiogroup"
                    aria-labelledby={`${titleId}-plan-label`}
                  ...
                  >
                    {CERTIFICATE_PLANS.map((plan) => {
                      ...
                      return (
                        <label
                          key={plan.id}
                          className={`p-3.5 rounded-lg border cursor-pointer transition-all duration-200 block has-[:focus-visible]:outline has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-offset-2 has-[:focus-visible]:outline-[#ea4043] ${
                            isSelected
                ...
                <div>
                  ...
                  <div
                    role="radiogroup"
                    aria-labelledby={`${titleId}-delivery-label`}
                    className="grid grid-cols-1 sm:grid-cols-2 gap-2.5"
                  >
                    {DELIVERY_OPTIONS.map((option) => (
                      <label
                        key={option.id}
                        className={`p-3.5 rounded-lg border text-left cursor-pointer transition-all block has-[:focus-visible]:outline has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-offset-2 has-[:focus-visible]:outline-[#ea4043] ${
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor={`${titleId}-recipientName`} className="block text-xs uppercase tracking-wider text-[#8e8e93] font-semibold mb-1.5">
                      {isRu ? "Имя получателя" : "Recipient Name"}
                    </label>
                    <input
                      id={`${titleId}-recipientName`}
                      name="recipientName"
                      autoComplete="name"
                      type="text"
                  ...
                  <div>
                    <label htmlFor={`${titleId}-buyerName`} className="block text-xs uppercase tracking-wider text-[#8e8e93] font-semibold mb-1.5">
                      {isRu ? "Ваше имя *" : "Your Name *"}
                    </label>
                    <input
                      id={`${titleId}-buyerName`}
                      name="buyerName"
                      autoComplete="name"
                      type="text"
                      required
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor={`${titleId}-phone`} className="block text-xs uppercase tracking-wider text-[#8e8e93] font-semibold mb-1.5">
                  ...
                  <div>
                    <label htmlFor={`${titleId}-email`} className="block text-xs uppercase tracking-wider text-[#8e8e93] font-semibold mb-1.5">
                      Email *
                    </label>
                    <input
                      id={`${titleId}-email`}
